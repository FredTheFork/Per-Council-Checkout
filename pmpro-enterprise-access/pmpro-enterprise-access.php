...
function pmpe_nuclear_css_injection() {
    ...
    if (function_exists('pi_checkout_core_asset_url')) {
        ...
        $enterprise_url = pi_checkout_core_asset_url('pi-checkout-enterprise.min.css');
        ...
        echo '<link rel="stylesheet" id="pi-checkout-enterprise-nuclear" href="' . esc_url($enterprise_url) . '?v=' . time() . '" type="text/css" media="all" />' . "\n";

        foreach (['pi-checkout-tokens.min.css', 'pi-checkout-base.min.css', 'pi-checkout-enterprise.min.css'] as $f) {
...
define( 'PMPE_LEVEL_ID', intval(get_option('pmpe_enterprise_checkout_level_id', 0)) );

// Get enterprise price from the PMPro level
function pmpe_get_price() {
    $level_id = intval(get_option('pmpe_enterprise_checkout_level_id', 0));
    if (!$level_id || !function_exists('pmpro_getLevel')) return 0;
    $level = pmpro_getLevel($level_id);
    return $level ? floatval($level->initial_payment) : 0;
}
define( 'PMPE_PRICE', pmpe_get_price() );
...
add_action('wp_enqueue_scripts', function () {
    ...
    $checkout_handle = $use_core ? 'pi-checkout-js' : 'pmpe-checkout-js';
    ...
    if ($use_core) {
        ...
        // Fallback: load per-council base CSS then enterprise overrides
        if (file_exists(WP_PLUGIN_DIR . '/pmpro-per-council/assets/pmpc-style.css')) {
            wp_enqueue_style('pmpc-checkout-css', plugins_url('pmpro-per-council/assets/pmpc-style.css'), [], '2.2.0');
        }
        wp_enqueue_style('pmpe-checkout-css', plugin_dir_url(__FILE__) . 'assets/pmpe-checkout.css', ['pmpc-checkout-css'], '2.0.0');
        wp_enqueue_script('pmpe-checkout-js', plugin_dir_url(__FILE__) . 'assets/pmpe-checkout.js', ['jquery'], '2.0.0', true);
    }
    ...
    wp_localize_script($checkout_handle, 'pmpeConfig', [
