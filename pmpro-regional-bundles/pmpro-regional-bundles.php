...
function pmrb_nuclear_css_injection() {
    ...
    // Use unified pi-checkout-core assets if available, fall back to local
    if (function_exists('pi_checkout_core_asset_url')) {
        $tokens_url   = pi_checkout_core_asset_url('pi-checkout-tokens.min.css');
        $base_url     = pi_checkout_core_asset_url('pi-checkout-base.min.css');
        $regional_url = pi_checkout_core_asset_url('pi-checkout-regional.min.css');
        echo '<link rel="stylesheet" id="pi-checkout-tokens-nuclear" href="' . esc_url($tokens_url) . '?v=' . time() . '" type="text/css" media="all" />' . "\n";
        echo '<link rel="stylesheet" id="pi-checkout-base-nuclear" href="' . esc_url($base_url) . '?v=' . time() . '" type="text/css" media="all" />' . "\n";
        echo '<link rel="stylesheet" id="pi-checkout-regional-nuclear" href="' . esc_url($regional_url) . '?v=' . time() . '" type="text/css" media="all" />' . "\n";

        foreach (['pi-checkout-tokens.min.css', 'pi-checkout-base.min.css', 'pi-checkout-regional.min.css'] as $f) {
            $p = pi_checkout_core_asset_path($f);
            if (file_exists($p)) {
                echo '<style id="' . str_replace('.', '-', $f) . '-inline">' . file_get_contents($p) . '</style>' . "\n";
            }
        }
    } else {
...
define('PMRB_LEVEL_ID', intval(get_option('pmrb_region_level_id', 0)));

// Get bundle price from the first region bundle
function pmrb_get_bundle_price() {
    if (function_exists('pis_get_region_bundles')) {
        $bundles = pis_get_region_bundles();
        foreach ($bundles as $region => $meta) {
            return floatval($meta['price'] ?? 0);
        }
    }
    return 0;
}
define('PMRB_BUNDLE_PRICE', pmrb_get_bundle_price());

// Return regions as [region_id => region_name] for template use
function pmrb_get_regions() {
    if (!function_exists('pis_get_region_bundles')) return [];
    $bundles = pis_get_region_bundles();
    $regions = [];
    foreach ($bundles as $region_name => $meta) {
        $regions[$region_name] = $region_name;
    }
    return $regions;
}

// Return all councils across all bundles as [{id, name, region}] for template use
function pmrb_get_all_councils() {
    if (!function_exists('pis_get_region_bundles')) return [];
    $bundles = pis_get_region_bundles();
    $councils = [];
    foreach ($bundles as $region_name => $meta) {
        foreach (($meta['councils'] ?? []) as $council_name) {
            $councils[] = [
                'id' => $council_name,
                'name' => $council_name,
                'region' => $region_name,
...
add_action('wp_enqueue_scripts', function () {
    ...
    // Unified checkout assets from pi-checkout-core plugin (falls back to local assets)
    $use_core = function_exists('pi_checkout_core_asset_url');
    $checkout_handle = $use_core ? 'pi-checkout-js' : 'pmrb-checkout-js';
    ...
    if ($use_core) {
        ...
        wp_enqueue_script('pi-checkout-js', pi_checkout_core_asset_url('pi-checkout.min.js'), ['jquery'], '1.0.0', true);
    } else {
        wp_enqueue_script('pmrb-checkout-js', plugin_dir_url(__FILE__) . 'assets/pmrb-checkout.js', ['jquery'], '3.0.0', true);
    }
    ...
    wp_localize_script($checkout_handle, 'pmrbConfig', [
        'regions' => $regions,
        'prices' => $prices,
    ...
    // Unified piCheckoutConfig for the shared JS engine
    if ($use_core) {
        wp_localize_script('pi-checkout-js', 'piCheckoutConfig', [
            'type' => 'regional',
            'totalSteps' => PMRB_TOTAL_STEPS,
            'prefix' => 'pmrb',
            'regions' => $regions,
    ...
    if (!$use_core) {
        wp_enqueue_style('pmrb-checkout-css', plugin_dir_url(__FILE__) . 'assets/pmrb-style.css', [], '3.0.0');
    }
...
JS;
    ...
    wp_add_inline_script($checkout_handle, $inline_js);
});
