...
class PIE_PmproHooks
{
    ...
    public static function handle_stripe_success(): void
    {
        ...
        if (empty($session_id)) {
            error_log('[PIE] handle_stripe_success: missing session_id');
            return;
        }

        // Idempotency guard — don't re-process if user refreshes the page
        $processed_flag = 'pie_stripe_done_' . $session_id;
        ...
        // Try URL parameter first (we add pie_key to the success_url)
        if (isset($_GET['pie_key'])) {
            $transient_key = sanitize_text_field($_GET['pie_key']);
        }
        ...
        if (empty($secret_key)) {
            error_log('[PIE] handle_stripe_success: no Stripe secret key found');
        ...
        if (!empty($session_id)) {
            ...
            if (is_wp_error($response)) {
                error_log('[PIE] handle_stripe_success: Stripe API error: ' . $response->get_error_message());
            } else {
        ...
        if (!empty($stripe_session)) {
            ...
            if (!$is_paid && !$is_complete) {
                error_log('[PIE] handle_stripe_success: payment not complete. payment_status=' . ($stripe_session['payment_status'] ?? 'unknown') . ' status=' . ($stripe_session['status'] ?? 'unknown'));
                wp_safe_redirect(home_url('/membership-account/'));
                exit;
            }
            ...
            if (empty($session_meta)) {
                ...
                $transient_key = $meta['pie_session_key'] ?? ($stripe_session['client_reference_id'] ?? '');
                if (!empty($transient_key)) {
                    $session_meta = get_transient($transient_key);
                }
                if (empty($session_meta)) {
        ...
        if (empty($session_meta) || !is_array($session_meta)) {
            error_log('[PIE] handle_stripe_success: no session metadata found for session ' . $session_id);
            wp_safe_redirect(home_url('/membership-account/'));
            exit;
        }
        ...
        if ($logged_in && $existing_user_id > 0) {
            ...
            if ($user_id === 0 && !empty($email)) {
                ...
                if (is_wp_error($user_id)) {
                    error_log('[PIE] handle_stripe_success: wp_create_user failed for ' . $email . ': ' . $user_id->get_error_message());
                    $user_id = 0;
                }
            }
        }
        ...
        if ($user_id <= 0) {
            error_log('[PIE] handle_stripe_success: user_id is 0 after creation attempt');
            wp_safe_redirect(home_url('/membership-account/'));
            exit;
        }
        ...
        error_log('[PIE] handle_stripe_success: success — user_id=' . $user_id . ' level=' . $level_id . ' price=' . $price);
