...
class PIE_CheckoutDetection
{
    ...
    {
        return self::is_enterprise_checkout();
    }

    public static function is_enterprise_checkout(): bool
    {
        if (self::$cached_result !== null) {
            return self::$cached_result;
        }

        if (!empty($_REQUEST['pie_complete'])) {
        ...
        self::$cached_result = ($current_level === $configured_level) || ($current_level === 60);
        return self::$cached_result;
    }
    ...
    public static function has_checkout_shortcode(): bool
    {
        ...
        return strpos($post->post_content, 'enterprise_checkout') !== false;
    }
    ...
    {
        return '<div id="pmpe-checkout-root">'
            ...
            . '<div style="width:44px;height:44px;border:3px solid #e2e8f0;border-top-color:#0c8fe6;border-radius:50%;animation:pie-spin 0.8s linear infinite;margin:0 auto 16px;"></div>'
            . '<p style="font-size:14px;margin:0;">Loading checkout...</p>'
            . '</div>'
            . '</div>'
            . '<style>@keyframes pie-spin{to{transform:rotate(360deg)}}</style>'
            . '</div>';
    }
