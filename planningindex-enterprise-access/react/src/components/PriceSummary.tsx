...
import { Globe, FileText, Check, Users } from 'lucide-react';

export function PriceSummary() {
  const { selectedTemplateId, teamSeats, monthlyCost, totalDueToday } = useCheckout();
    ...
    <div className="card sticky top-6 overflow-hidden">
      ...
      <div className="px-5 py-4">
        <div className="mb-4">
          ...
          <div className="rounded-lg bg-brand-50 px-3 py-2">
            <p className="text-sm font-semibold text-brand-700">Enterprise Access</p>
            <p className="mt-0.5 text-xs text-brand-600">All UK Councils included</p>
          </div>
          <div className="mt-2 flex items-center gap-1.5 text-sm">
            <Check className="h-3 w-3 shrink-0 text-success-500" />
            <span className="text-slate-600">Unlimited nationwide access</span>
          </div>
        </div>
        ...
        <div className="mb-4 border-t border-slate-100 pt-4">
          ...
          <div className="flex items-start gap-2 text-sm">
            ...
            <div>
              <p className="font-medium text-slate-700">{teamSeats} {teamSeats === 1 ? 'seat' : 'seats'}</p>
              <p className="text-xs text-slate-400">Included with Enterprise plan</p>
            </div>
          </div>
        </div>
      ...
      <div className="border-t border-slate-200/60 bg-slate-50/50 px-5 py-4">
        <div className="mb-1 flex items-center justify-between">
          ...
          <span className="text-lg font-bold text-slate-900">£{(monthlyCost || enterprisePrice).toFixed(2)}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-slate-600">Total Due Today</span>
          <span className="text-lg font-bold text-brand-600">£{(totalDueToday || enterprisePrice).toFixed(2)}</span>
        </div>
