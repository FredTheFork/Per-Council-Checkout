/**
 * API client for the Planning Index trial checkout.
 *
 ...
 * window.PlanningIndexTrialCheckout.nonce.
 *
...
function getConfig(): PitConfig {
  const cfg = (window as unknown as { PlanningIndexTrialCheckout?: PitConfig }).PlanningIndexTrialCheckout;
  if (!cfg || !cfg.apiBase || !cfg.nonce) {
    console.warn('[PIT] No valid window.PlanningIndexTrialCheckout config found, using dev fallback. window.PlanningIndexTrialCheckout =', cfg);
  ...
  console.log('[PIT] Config loaded from window.PlanningIndexTrialCheckout:', { apiBase: cfg.apiBase, isLoggedIn: cfg.isLoggedIn, userId: cfg.userId, levelId: cfg.levelId, gateway: cfg.gateway });
  return cfg;
}
...
async function request<T>(
  ...
  if (!res.ok) {
    ...
    try {
      ...
      console.error('[PIT] API error response:', err);
      if (err.message) message = err.message;
      else if (err.data && err.data.message) message = err.data.message;
    } catch {
      console.error('[PIT] Non-JSON error response, status:', res.status);
    }
  ...
  if (json && typeof json === 'object' && 'success' in json && (json as { success: boolean }).success === false) {
    ...
    console.error('[PIT] API returned success: false:', json);
    throw new Error(msg);
  }
  return json;
}
...
export const api = {
  ...
  /**
   * POST /checkout — start the free trial checkout.
   *
   * Unlike the paid per-council checkout, this does NOT redirect to
   * Stripe. The PMPro checkout processes at £0 and the user gets
   * 14 days of free access before needing to subscribe.
   */
  async startTrial(): Promise<TrialCheckoutResponse> {
    if (isDevMode()) {
      return {
        success: true,
        orderCode: 'PIT-DEV' + Math.random().toString(36).substring(2, 8).toUpperCase(),
        orderDate: new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }),
        planName: 'Planning Index Free Trial',
        councilCount: 0,
        monthlyCost: 0,
        totalDueToday: 0,
        trialDays: 14,
    ...
    return request<TrialCheckoutResponse>('/checkout', { method: 'POST' });
  },

  /** GET /checkout/verify-price — server-sourced price confirmation (£0 for trial). */
...
export function getInjectedConfig(): PitConfig {
  return getConfig();
}
