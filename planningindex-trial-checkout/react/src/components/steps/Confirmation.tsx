...
import { ArrowLeft, ArrowRight, Building, Mail, Phone, MapPin, Loader as Loader2, User, FileText, MapPinned, Gift, Clock } from 'lucide-react';
...
export function Confirmation() {
  ...
  const handleStartTrial = async () => {
    ...
    // Guard: must have at least 1 council selected
    if (selectedCouncils.length < 1) {
      setError('Please go back and select at least 1 council before starting your free trial.');
      setLoading(false);
      return;
    }

    // Guard: must not exceed 5 councils
    if (selectedCouncils.length > 5) {
      setError('You can select up to 5 councils for your free trial. Please go back and remove some.');
      setLoading(false);
      return;
    }
    ...
    if (!isUserLoggedIn() && !accountInfo?.email) {
      setError('Please go back and complete your account information before starting your trial.');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Start the free trial — this redirects to the PMPro checkout
      // page which processes at £0 (no Stripe redirect).
      console.log('[PIT] Calling startTrial with:', {
      ...
      console.log('[PIT] startTrial result:', result);

      if (!result.success || !result.redirectUrl) {
        throw new Error(result.message || 'Unable to start your free trial.');
      }

      // Redirect to the PMPro checkout page to complete account creation
      window.location.href = result.redirectUrl;
    } catch (err) {
      console.error('[PIT] trial checkout error:', err);
      const message = err instanceof Error ? err.message : 'An error occurred while starting your trial.';
      setError(message);
      setLoading(false);
    }
  };
      ...
      <div>
        <div className="mb-6">
          ...
          <p className="mt-2 text-sm text-slate-500 sm:text-base">
            Review your selections and add optional business information. No payment is required — you get{' '}
            <span className="font-semibold text-success-700">{trialDays} days free</span>.
          </p>
        </div>
        ...
        <div className="card mb-6 p-5">
          <div className="mb-4 flex items-center justify-between border-b border-slate-100 pb-4">
            <div>
              <p className="text-sm text-slate-500">Trial Duration</p>
              <p className="font-display text-2xl font-bold text-success-600">
                {trialDays} days
                <span className="text-base font-medium text-slate-400"> free</span>
              </p>
            </div>
          ...
          <div className="mb-4 rounded-lg bg-success-50 px-4 py-3 ring-1 ring-inset ring-success-200">
            <div className="flex items-start gap-2">
              <Gift className="mt-0.5 h-4 w-4 shrink-0 text-success-600" />
              <p className="text-sm text-success-700">
                After your {trialDays}-day trial ends, you'll be charged{' '}
                <span className="font-semibold">£3/council/month</span> for your selected councils.
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <button onClick={handleStartTrial} disabled={loading} className="btn-primary">
            {loading ? (
              <>
