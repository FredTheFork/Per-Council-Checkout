...
class PIT_CheckoutDetection
{
    ...
    public static function has_checkout_shortcode(): bool
    {
        ...
        return strpos($post->post_content, 'planningindex_trial_checkout') !== false;
    }
    ...
    public static function render_root_div(): string
    {
        return '<div id="planning-checkout-root">'
            ...
            . '<div style="width:40px;height:40px;border:3px solid #e2e8f0;border-top-color:#2563eb;border-radius:50%;animation:pit-spin 0.8s linear infinite;margin:0 auto 16px;"></div>'
            . '<p style="font-size:14px;">Loading trial checkout...</p>'
            . '</div>'
            . '</div>'
            . '<style>@keyframes pit-spin{to{transform:rotate(360deg)}}</style>'
            . '</div>';
    }
